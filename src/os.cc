...
void
x86_linux_os::char2regset(char *buf, int size, RegisterSet *reg) {
	if (bit == 32) {
...
void
x86_linux_os::regset2char(char *buf, int size, RegisterSet *reg) {
	if (bit == 32) {
		if (size == sizeof (struct i386_linux_regs)) {
			struct i386_linux_regs	*regs_buf = (struct i386_linux_regs *)buf;
...
void
x86_linux_os::char2fpregset(char *buf, int size, RegisterSet *reg) {
	unsigned char			*p;
...
void
x86_linux_os::fpregset2char(char *buf, int size, RegisterSet *reg) {
	unsigned char			*p;
	std::vector<unsigned char>	val;
...
void
x86_freebsd_os::char2regset(char *buf, int size, RegisterSet *reg) {
	if (bit == 32) {
...
void
x86_freebsd_os::regset2char(char *buf, int size, RegisterSet *reg) {
	if (bit == 32) {
		struct i386_freebsd_regs	*regs_buf = (struct i386_freebsd_regs *)buf;
