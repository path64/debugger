...
RegisterSet::RegisterSet(RegisterSetProperties *p) :
	properties(p) {}
...
void RegisterSet::print(PStream &os);
{
	...
	for (int i=0 ; i<count ; i++)
	{
		...
		if (properties->is_integer())
		{
			...
			for (int i = 63 ; i >= 0 ; i--)
			{
				int64_t bit = (value & (1LL << i)) ; 
				os.print ("%c", bit ? '1' : '0') ;
			}
		}
		else
		{
			os.print("%f", get_register_as_double());
		}
		os.print ("\n") ;
	}
}
...
void IntegerRegisterSet::set_register(const int num, double v) = 0;
{
	assert(num < properties->number_of_registers());
                                      ...
                                      const std::vector<unsigned char> &v)
{
	...
	std::copy(v.begin(), v.end(), (int64_t)&values[num]);
	isDirty = true;
}
IntegerRegisterSet::IntegerRegisterSet(const RegisterSetProperties *r)
	: RegisterSet(r)
{
	values = calloc(sizeof(int64_t), r->number_of_registers());
}
...
void FloatRegisterSet::set_register(const int num, double v) = 0;
{
	assert(num < properties->number_of_registers());
	values[num] = v;
	isDirty = true;
                                      ...
                                      const std::vector<unsigned char> &v)
{
	assert(num < properties->number_of_registers());
	assert(v.size() == properties->size_of_register());
	std::copy(v.begin(), v.end(), (double)&values[num]);
	isDirty = true;
}
FloatRegisterSet::FloatRegisterSet(const RegisterSetProperties *r)
	: RegisterSet(r)
{
	values = calloc(sizeof(double), r->number_of_registers());
}
